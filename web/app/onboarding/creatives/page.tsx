...
export default function CreativesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Launch needs the Meta connection; the owner may have finished onboarding without
      // it (ops connects it for them). Never show a launch button that can only fail.
      api.onboardingStatus()
        .then((st) => setMetaMissing(!!st.missing_steps?.includes("meta_connection")))
        .catch(() => {});
      api.home(account).then((h) => setPhase(h.phase)).catch(() => {});
    } catch (e: any) {
  ...
  // Queued mode: creatives are generated by a worker — poll instead of a dead
  // "check back in a minute" note.
  useEffect(() => {
  ...
  async function launch() {
    ...
    try {
      ...
      const res: any = await api.launch(getUser()!.account_id!);
      if (res?.status === "queued") {
        // Worker mode: launch is in flight — no premature "live!" celebration.
        toast.show(t("creatives.launchQueued", "Launching — I'll notify you when live."), "success");
  ...
  return (
    <main className="min-h-[100dvh] pb-28">
      ...
      {phase && ["LIVE", "OPTIMIZING", "FATIGUE_REFRESH"].includes(phase) ? (
        // Already live — revisiting this page must read as review, not as a re-launch.
        <div className="cta-dock">
          <Button fullWidth size="lg" variant="secondary" onClick={() => router.push("/ads")}>
            {t("creatives.alreadyLive", "Your ads are live — see them")}
          </Button>
        </div>
      ) : metaMissing ? (
        <div className="cta-dock">
          <p className="mb-2 text-center text-xs text-ink-muted">
            {t("creatives.metaPending",
               "Saathi's team is connecting Facebook for you — nothing needed from you. Your approved ads launch automatically.")}
          </p>
          <Button fullWidth size="lg" variant="secondary" disabled={blocked} loading={busy}
            onClick={async () => {
              // Approve now; the launch cron fires the moment Meta is connected.
              const toApprove = kept.filter(
                (c) => c.compliance_status === "PASSED" && c.approval_status !== "APPROVED_FOR_LAUNCH");
              setBusy(true);
              try {
                await Promise.all(toApprove.map((c) => api.approveCreative(c.id)));
                toast.show(t("creatives.approvedWaiting", "Approved! Ads launch once Facebook is connected."), "success");
                router.replace("/dashboard");
              } finally { setBusy(false); }
            }}>
            {t("creatives.approveOnly", "Approve my ads")}
          </Button>
        </div>
      ) : (
      <div className="cta-dock">
