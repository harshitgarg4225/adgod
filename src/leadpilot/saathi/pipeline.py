...
def run_creative(session: Session, *, tenant_id: UUID, account_id: UUID, max_angles: int = 3) -> list[UUID]:
    ...
    for angle in angles:
        out = MakerAgent().run(
            ...
            context={"language": account.default_language, "angle": angle.title,
                     "brief": brief.offer if brief else "", "winners": []},
        )
    ...
    # Trust gate: full autopilot auto-approves; otherwise owner reviews.
    account.phase = (
        AccountPhase.CREATIVE_GENERATED.value if full_autopilot
        else AccountPhase.PENDING_APPROVAL.value
    )
...
def launch_campaigns(session: Session, *, tenant_id: UUID, account_id: UUID) -> list[UUID]:
    ...
    if not creatives:
        raise ValueError("no compliant creatives to launch")
    ...
    for adset in ad_sets:
        ...
        if action == OptimizationAction.PAUSE:
            ...
            session.add(Notification(
                ...
                title="Refreshing tired ads", body="Saathi is creating fresh creative for a fatigued ad set."))
